...
async def test_create_success(service):
    ...
    ep = await service.create_for_service(1, EndpointRequest(url="http://x.test/h"))
    assert ep.id == 1
    assert ep.service_id == 1
...
async def test_create_invalid_url_no_scheme():
    with pytest.raises(ValidationError) as exc_info:
        EndpointRequest(url="example.com/h")
    assert "http://" in str(exc_info.value)


async def test_create_invalid_url_empty():
    with pytest.raises(ValidationError):
        EndpointRequest(url="   ")


async def test_create_invalid_url_too_long():
    long_url = "http://" + ("a" * 2050)
    with pytest.raises(ValidationError):
        EndpointRequest(url=long_url)
...
async def test_list_for_service_returns_endpoints(service):
    service.service_repo.get_by_id.return_value = AsyncMock(id=1)
...
async def test_update_invalid_url_raises():
    with pytest.raises(ValidationError):
...
async def test_update_only_is_active(service):
    ...
    ep = await service.update(1, EndpointUpdateRequest(is_active=False))
